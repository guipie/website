  ...
  <div class="kindeditor">
    <div>
      <VolBox icon="ios-chatbubbles" :model.sync="uploadModel" title="图片上传" :height="180" :width="520" :padding="15">
        <VolUpload style="text-align: center; border: 1px dotted #FF9800;padding: 20px;" :autoUpload="true" :multiple="true"
          :url="UploadImgUrl" :max-file="uploadCount" :img="true" :upload-after="uploadAfter" :upload-before="uploadBefore"></VolUpload>
      </VolBox>
    </div>
    ...
    methods: {
      ...
      uploadAfter(result, files) {
        ...
        let urls = files.map((x) => {
          return (
            `<img src="` + (this.http.ipAddress + result.data) + `"/>`
          );
        });
