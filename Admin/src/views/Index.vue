<template>
  <div id="vol-container" :class="['vol-theme-' + theme]">
    <div class="vol-aside">
      <div class="header">
        <span class="header-font">诡瞥社区后台</span>
        <!-- <img v-bind:src="log" /> -->
      </div>
      <div class="vol-menu">
    ...
    <div class="vol-container">
      <div class="vol-header">
        ...
        <div class="header-info">
          <div class="h-link">
            <ul>
              <li v-for="(item, index) in links" :key="index" v-bind:class="{ actived: selectId == item.id }">
      ...
      <div class="vol-path">
        ...
        <Tabs @on-click="selectNav" :before-remove="removeNav" v-model="selectId" type="card" :animated="false" class="header-navigation">
          <!-- 2020.07.31增加手动打开tabs -->
          <TabPane :class="{ active: navIndex == selectId }" :name="item.navIndex" :closable="navIndex != 0" v-for="(item, navIndex) in navigation" :key="navIndex" :label="item.name"></TabPane>
        </Tabs>
      </div>
    ...
    <Drawer class="theme-selector" title="选择皮肤颜色" :closable="false" v-model="theme_moel">
      <div @click="changeThen(item.name)" class="item" v-for="(item, index) in theme_color" :key="index" :style="{ background: item.color }"></div>
    </Drawer>
  </div>
</template>
<script>
...
export default {
  ...
  created() {
    let theme = localStorage.getItem("vol_theme") || "dark";
    if (theme) {
      this.theme = theme;
    }
...
.vol-theme-dark .vol-aside .vol-menu >>> .ivu-menu-item-active.ivu-menu-item-selected {
  color: white !important;
