  ...
  <div style="padding-top:20px;">
    <el-form :model="model" status-icon label-width="100px" :rules="rules" ref="contentRef">
      ...
      <el-form-item v-if="showMore">
        ...
        <el-row :span="24">
          <el-col :span="8">
            <el-image v-for="url in  coversImgs" :key="url" style="width: 100px;margin:10px;" :src="url">
            </el-image>
            <el-tooltip effect="dark" content="封面图片" placement="bottom">
              <el-image @click="coversDialog" style="width: 80px;cursor:pointer;" src="/upload.jpg" fit="fill">
              </el-image>
            </el-tooltip>
          </el-col>
          <el-col :span="8">
            ...
            <div style="clear:both">
              <el-tag v-for="(tag,index) in tags" :key="index" closable @close="tags.splice(index,1)" type="info">
                #{{tag}}
              </el-tag>
            </div>
          </el-col>
    ...
    <el-dialog :visible.sync="coversVisible">
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          <el-upload ref="coversRef" list-type="picture-card" :on-remove="coverRemove" :limit="3" action="/"
            :http-request="coverUpload">
            <i class="el-icon-plus"></i>
          </el-upload>
        </div>
        <div class="text item">
          <el-image @click="coverConfirm(url)" v-for="url in contentImgs" :key="url"
            style="width: 100px;margin:10px;cursor:pointer;" :src="url">
          </el-image>
        </div>
      </el-card>
...
export default {
  ...
  methods: {
    ...
    coverRemove (file) {
      ...
      if (delIndex > -1)
        this.$refs.coversRef.fileList.splice(delIndex, 1);
    },
    coverUpload (params) {
      let _this = this,
        file = params.file,
